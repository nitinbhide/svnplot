...
class SVNSqlite2Ora:
	...
	for row in cur:
                ...
                for row2 in cur2:
						...
						mat[u[address]][f[msgID]] = mat[u[address]][f[msgID]] + 1
	...
	for row in cur:
                ...
                for row2 in cur2:
						...
						mat2[f[backlink]][u[address]] = mat2[f[backlink]][u[address]] + 1
	...
	for i in u:
                for j in u:
			if (i <> j):		
				output.write("<link source=\""+ "%s" % i + "\" target=\"" + "%s" % j + "\" value=\"" + "%i" % mat12[u[i]][u[j]] + "\"/>\n")
