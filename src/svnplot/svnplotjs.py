...
class SVNPlotJS(SVNPlotBase):
	...
	def ActivityByWeekdayAll(self):
		...
		graph.data(labels, data, name="Number of Commits")
	...
	def CommitActivityIdxGraph(self):
		...
		graph.data(cmdates, temperaturelist, name="Activity Index")
		return graph
	...
	def LocGraph(self):
		...
		graph.data(dates, loc, name="Number of Lines")
		return graph
	...
	def LoCDevContributionGraph(self):
		...
		for i, author in enumerate(authList):
			dates, loc = self.svnstats.getLoCTrendForAuthor(author)
			if( len(dates) > 0):
				x_name = 'x%d' % i
				graph.addDataSeries(dates, name=x_name)
				graph.addDataSeries(loc, name=author, mapto=x_name)
	...
	def LocChurnGraph(self):
		...
		graph.addDataSeries(loc, name="LoC", mapto='x')
		graph.addDataSeries(churnlist, name="Churn", mapto='x', axis='y2', color='red')
		graph.addDataSeries(dates, name='x')
	...
	def FileCountGraph(self):
		...
		graph.data(dates, fclist, name="Number of files")
	...
	def FileTypesGraph(self):
		...
		graph.data(ftypelist, ftypecountlist)
	...
	def AvgFileLocGraph(self):
		...
		graph.data(dates, avgloclist, "Average line count")
		return graph
	...
	def DirectorySizePieGraph(self, depth=2, maxdircount=10):
		...
		graph.data(dirlist, dirsizelist, name="Size of files")
	...
	def DirFileCountPieGraph(self, depth=2, maxdircount=10):
		...
		graph.data(dirlist, dirsizelist, name="Number of files")		
		return graph
	...
	def DirectorySizeLineGraph(self, depth=2, maxdircount=10):
		...
		for i, dirname in enumerate(dirlist):
			...
			x_name = 'x%d' % i
			graph.addDataSeries(dates, name=x_name)
			graph.addDataSeries(loclist, name=dirname, mapto=x_name)
	...
	def AuthorsCommitTrend(self):
		...
		graph.data(binlabels, data, "Commit Frequency")
		return graph
	...
	def DailyCommitCountGraph(self):
		...
		graph.data(datelist,cmitcountlist, "Commit Count")
		return graph
	...
	def WasteEffortTrend(self):
		...
		graph.addDataSeries(datelist)
		graph.addDataSeries(linesadded, name="Lines Added", mapto='x')
		graph.addDataSeries(linesdeleted, name="Lines Deleted", mapto='x')
		graph.addDataSeries(wasteratio, name="Waste Ratio", mapto='x', axis='y2', color='red')
