...
def getTagFontSize(freq, minFreqLog, maxFreqLog):
    ...
    # change minFreqLog in such a way smallest log(freq)-minFreqLog is greater
    # than 0
    ...
    fontsize = int(
        MINFONTSIZE + ((math.log(freq) - minFreqLog) * scalingFactor) + 0.5)
    # now round off to ensure that font size remains in MINFONTSIZE and
    # MAXFONTSIZE
...
def normalize_activityidx(actIdx, minActivity, maxActivity):
    ...
    if(actIdx - minActivity > 1.0):
        # log of actIdx-minactivity has to be +ve for subsequent computations
        # to work.
        normactidx = math.log(actIdx - minActivity) / \
            math.log(maxActivity - minActivity)
...
class SVNPlotBase(object):
    ...
    def BasicStats(self, basicStatsTmpl):
        ...
        basestats['FirstRevDate'] = basestats[
            'FirstRevDate'].strftime('%b %d, %Y %I:%M %p')
        basestats['LastRevDate'] = basestats[
            'LastRevDate'].strftime('%b %d, %Y %I:%M %p')
    ...
    def AuthorCloud(self, maxAuthCount=50):
        ...
        if(len(authCloud) > 0):
            ...
            # activity index
            authTagList = sorted(
                authCloud, key=operator.itemgetter(2), reverse=True)
            ...
            tagsData = [{'text': str(
                auth), 'count': freq, 'color': actIdx} for auth, freq, actIdx in authTagList]
    ...
    def TagCloud(self, numWords=50):
        ...
        if(len(words) > 0):
            ...
            # alphabetical order.
            tagWordList = sorted(
                tagWordList[0:numWords], key=operator.itemgetter(0))

            # Create a list of list for javascript input
            tagData = [
                {'text': makeunicode(x), 'count': freq} for x, freq in tagWordList]
