...
class SVNLogClient:
    ...
    def ssl_client_cert_password_prompt(self, realm, may_save):
        ...
        logging.debug(
            "callback_ssl_client_cert_password_prompt called to gain password for subversion in realm %s ." % (realm))
            ...
            for svnerr in errs:
                ...
                if(code == 22):
                    ...
                    if('APR_ICONV_PATH' in os.environ):
                        helpmsg = helpmsg + \
                            'Current value of APR_ICONV_PATH is %s' % os.environ[
                                'APR_ICONV_PATH']
                    else:
                        helpmsg = helpmsg + \
                            'Currently APR_ICONV_PATH is not set'
                    exitadvised = True
                elif (code == 145000):
                    '''
                    ...
                    helpmsg = helpmsg + \
                        "temp directory in windows and hence tried to create it 'c:\\'. With new version of windows"
                    helpmsg = helpmsg + \
                        ''' user doesnot have permissions to write in 'c:\'. \n'''
                    helpmsg = helpmsg + \
                        '''Please run svnlog2sqlite as 'administrator'.\n'''
                    helpmsg = helpmsg + \
                        '''Start svnlog2sqlite.py from command prompt with administrator privileges'''
    ...
    def findStartEndRev(self, startdate=None, enddate=None):
        ...
        if(not self.isRepoUrlSameAsRoot()):
            ...
            # for a given url, I am using headrevision and first revision time
            # to get those
    ...
    def getLastRevForDate(self, revdate, url, detailedlog=False):
        ...
        revend = pysvn.Revision(
            pysvn.opt_revision_kind.date, revdate + (24 * 60 * 60))
        revloglist = self.svnclient.log(url,
    ...
    def getLog(self, revno, url=None, detailedLog=False):
        ...
        logging.debug(
            "Trying to get revision log. revno:%d, url=%s" % (revno, url))
    ...
    def getLogs(self, startrevno, endrevno, cachesize=1, detailedLog=False):
        ...
        logging.debug(
            "Trying to get revision logs [%d:%d]" % (startrevno, endrevno))
        revlog = self.svnclient.log(url,
    ...
    def __isBinaryFile(self, filepath, revno):
        ...
        '''
        logging.debug(
            "Binary file check for file <%s> revision:%d" % (filepath, revno))
        # if explicit mime-type is not found always treat the file as 'text'
        binary = False
    ...
    def _getLineCount(self, filepath, revno):
        ...
        self.svnclient.export(
            url, dest_path=outpath, revision=rev, ignore_externals=True, recurse=False)
    ...
    def getRootUrl2(self):
        ...
        headrev = pysvn.Revision(pysvn.opt_revision_kind.head)
        urlinfo = self.svnclient.info2(
            possibleroot, revision=headrev, recurse=False)
    ...
    def getRootUrl(self):
        if(self.svnrooturl == None and self.svnclient.is_url(self.svnrepourl)):
            ...
            # Comment this line if PySVN - root_url_from_path() function works
            # for you.
            self.getRootUrl2()
