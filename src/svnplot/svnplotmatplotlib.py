...
--------------------------------------------------------------------------------------
SVNPlotMatplotLib implementation. Matplotlib based graph implementation.
'''
...
class SVNPlotMatplotLib(SVNPlotBase):
    ...
    def _addFigureLegend(self, ax, labels, loc="lower center", ncol=4):
        ...
        # then i get an error "range() step argument must not be zero" on line
        # 542 in legend.py
        if(len(labels) < ncol):
            ncol = len(labels)
    ...
    def _drawHBarGraph(self, datalist, labels, barwid):
        ...
        ax.barh(ylocations, datalist, height=barwid,
                color=self.clrlist[clridx])
    ...
    def _drawStackedHBarGraph(self, dataList, labels, legendlist, barwid):
        ...
        clridx = 0
        maxclridx = len(self.clrlist)
        ax.barh(ylocations, dataList[0], height=barwid, color=self.clrlist[
                clridx], label=legendlist[0])
    ...
    def _drawScatterPlot(self, dates, values, plotidx, plotcount, title, refaxs):
        if(refaxs == None):
            ...
            fig = plt.figure()
            # 1 inch height for each author graph. So recalculate with height.
            # Else y-scale get mixed.
            figHt = float(self.commitGraphHtPerAuthor * plotcount)
            fig.set_figheight(figHt)
            # since figureheight is in inches, set around maximum of 0.75
            # inches margin on top.
            topmarginfrac = min(0.15, 0.85 / figHt)
            logging.debug("top/bottom margin fraction is %f" % topmarginfrac)
            fig.subplots_adjust(
                bottom=topmarginfrac, top=1.0 - topmarginfrac, left=0.05, right=0.95)
        else:
            fig = refaxs.figure

        axs = fig.add_subplot(
            plotcount, 1, plotidx, sharex=refaxs, sharey=refaxs)
        axs.grid(True)
    ...
    def _drawPieGraph(self, slicesizes, slicelabels):
        ...
        (patches, labeltext, autotexts) = axs.pie(
            slicesizes, labels=slicelabels, autopct='%1.1f%%')
