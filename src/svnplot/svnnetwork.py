...
class SVNNetworkBase(NX.Graph):
    ...
    def __init__(self, repodbpath=None, searchPath='/', repoUrl=None):
        ...
        # Minimum weight of edge should at least 2 (i.e. edges with weight 1
        # and 0 will be filtered out)
        self._minWt = 1.0
    ...
    def UpdateGraph(self):
        ...
        self.dbcon = sqlite3.connect(
            self._repodbpath, detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
        try:
    ...
    def _updateEdgeWtDecay(self, node1, node2, curdate):
        if(node1 != None and node2 != None):
            ...
            if newwt < 0:
                import pdb
                pdb.set_trace()
                newwt = getTemperatureAtTime(
                    curdate, lastdate, wt, COOLINGRATE)
            assert(newwt > 0.0)
    ...
    def _addEdgeWtDecay(self, node1, node2, curdate):
        ...
        if(node1 != None and node2 != None):
            ...
            if(lastdate != None):
                wt = self[node1][node2].get('weight', 1)
                wt = TEMPINCREMENT + \
                    getTemperatureAtTime(curdate, lastdate, wt, COOLINGRATE)
            self._edgedict[(node1, node2)] = curdate
            self._edgedict[(node2, node1)] = curdate
            assert(wt >= 0.0)
    ...
    def _updateCentrality(self, date):
        ...
        closecentrality = NX.closeness_centrality(
    ...
    def _addFileFileEdges(self):
        ...
        for filename1, filename2, weight in cur:
            ...
            # Remember each connection will appear twice (filenameABC,
            # filenameXYZ) and (filenameXYZ, filenameABC)
...
# break
    ...
    def filterEdges(self, condition=None):
        ...
        edges = [(node1, node2) for node1, node2, nodedata in self.edges_iter(
            None, data=True) if condition(node1, node2, nodedata['weight']) == True]
        if(len(edges) > 0):
    ...
    def filterEmptyNodes(self):
        ...
        nodes = [node for node in self.nodes_iter() if len(
    ...
    def getNodes(self, nodeType):
        nodes = [
            node for node in self.nodes_iter() if isinstance(node, nodeType) == True]
        return(nodes)
...
# self.remove_nodes_from(nodes)
    ...
    def _getLabelsList(self):
        labels = [node.name() for node in self.nodes_iter() if isinstance(
            node, SVNNetworkAuthorNode) == True]
        return(labels)
    ...
    def _drawAuthorNodes(self, pos):
        ...
        for node, clr in zip(authorNodes, clrlist):
            authors = [node]
            handles.append(
                NX.draw_networkx_nodes(self, pos, authors, with_labels=False, node_color=clr))
        axs = plt.gca()
        assert(axs != None)
        ...
        axs.legend(
            handles, labels, prop=legendfont, scatterpoints=1, markerscale=0.1)
        ...
        for idx in range(0, len(graphs)):
            ...
            if(subgraph.number_of_nodes() > minNodeCount):
                ...
                for node in subgraph.nodes_iter():
                    ...
                    nodeparentlist = ["subgraph %d" %
                                      (idx + 1)] + nodeparentlist
                    tmnode = root.addChild(nodeparentlist)
                    tmnode.setProp('commitcount', node.commitCount)
                    tmnode.setProp(
                        'centrality', NX.closeness_centrality(subgraph, node, weigthed))
